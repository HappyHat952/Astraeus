...
//STATE OF GAME
	...
	{
		p.addMessage(SpaceTrees.getStrategy(), Color.green);
		p.addMessage(p.getName().toUpperCase(), Color.green);
...
// GET UNITS Methods: Get specific unit(s)
	...
	public static Unit getNearestCriticalUnit(Unit origin, int radius) {
		...
		for (Unit u : targets) {
			float health = u.getCurEffectiveHealth()* u.getDodgeChance()*origin.getWeaponOne().getAccuracy();
			if (health < lowestHealth && !isPassive(u)) {
				lowestHealth = health;
				critical = u;
			}
		}
